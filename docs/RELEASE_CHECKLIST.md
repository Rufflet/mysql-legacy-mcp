...
- [x] Push the prepared `.github/workflows/ci.yml` and verify CI on Node 18.14.1, 22, and 24. Done: `gh auth status` shows the `workflow` scope, both workflow files are pushed and active (`gh workflow list`), and CI has completed successfully (`gh run list`) as of 2026-09-10 08:43 UTC — but only through commit `418def1`. The 0.2.0 work (write tools, pool, read-only transactions, and this documentation pass) was pushed afterward in the same session; check `git status -sb` / `gh run list` for whether CI has run against the current `main` tip.
