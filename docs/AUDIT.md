...
On 2026-09-10, authenticated GitHub API access confirmed the personal account Rufflet. The public repository is https://github.com/Rufflet/mysql-legacy-mcp. Package author, repository, homepage, and issues links are populated, and private: true has been removed. `mysql-legacy-mcp@0.2.0` was published to npm the same day (see RELEASE_CHECKLIST.md for the publish and Trusted Publishing setup). A server.json draft and matching mcpName prepare a future submission to the separate official MCP Registry (`registry.modelcontextprotocol.io`, tracked as "Phase 2" in RELEASE_CHECKLIST.md); that registry submission has not occurred.
